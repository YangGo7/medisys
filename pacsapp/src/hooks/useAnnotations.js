...
const useAnnotations = (currentStudyUID, setAnalysisStatus, setActiveLayer) => {
    ...
    const saveBoundingBox = useCallback((label) => {
        ...
        const normalizedBox = {
            ...
            confidence: 1.0,
            created: new Date().toISOString(),
            doctor_name: '새 어노테이션'  // 👈 추가: 임시값, 저장 후 서버에서 실제 값으로 업데이트됨
        };
    ...
    const saveAnnotationsToServer = useCallback(async () => {
        ...
        try {
            ...
            if (data.status === 'success') {
                // 👈 저장 성공 후 실제 판독의 이름으로 업데이트
                if (data.data && data.data.doctor_name) {
                    setAnnotationBoxes(prev => 
                        prev.map(box => ({
                            ...box,
                            doctor_name: data.data.doctor_name  // 실제 판독의 이름으로 업데이트
                        }))
                    );
                }
        ...
        try {
            ...
            if (data.status === 'success' && data.annotations) {
                const loadedBoxes = data.annotations.map((ann, index) => ({
                    ...
                    confidence: ann.confidence,
                    created: ann.created || new Date().toISOString(),
                    doctor_name: ann.doctor_name || 'DR001 - 김영상'  // 👈 추가: 서버에서 온 실제 판독의 이름 사용
                }));
